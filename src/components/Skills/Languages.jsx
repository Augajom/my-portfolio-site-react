...
const SkillCard = ({ icon, title, level, delay }) => (
  ...
  >
    ...
    <span className="text-[10px] font-mono text-gray-500 border border-white/5 px-2 py-0.5 rounded-md uppercase tracking-wider text-center whitespace-pre-line">
      {level}
    </span>
  </motion.div>
);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {items.map((key, idx) => (
            <SkillCard
              ...
              delay={idx}
              icon={iconsMap[key]}
              title={t(`skills.languages.items.${key}.title`)}
              level={t(`skills.languages.items.${key}.level`)}
            />
          ))}
        </div>
      </div>
